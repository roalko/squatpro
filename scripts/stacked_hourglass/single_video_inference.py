...
def load_model(model_json, model_weights, img, threshold = 0.05, num_stack = 4, num_class = 16, tiny = False):
    if tiny:
        xnet = HourglassNet(num_classes = num_class, num_stacks = num_stack, num_channels = 128, inres = (192, 192), outres = (48, 48))
    else:
        xnet = HourglassNet(num_classes = num_class, num_stacks = num_stack, num_channels = 256, inres = (256, 256), outres = (256, 256))

    xnet.load_model(model_json, model_weights)
    return xnet

def inference(xnet, model_json, model_weights, img, threshold = 0.2, num_stack = 4, num_class = 16, tiny = False):
...
for vid_num in range(1,101):
    ...
    for frameId, frame in enumerate(cap):
        cv2.imwrite('../../raw_data/out.jpg', cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))
        ...
        dim = (width, height)
        cv2.imshow('output', cv2.resize(frame, dim))
        cv2.waitKey(1)
